...
hold off
title(sprintf('Fiji plugin %s: %s, Traj>%d, R^2>%s', type, FittingTypeS, Mini_Trajlength, R_thresholdS));
xlabel('x (um)') 
ylabel('y (um)')
saveas(gcf, sprintf('PlotTrajFromFiji_%s_%s_Traj>%d_R>%s_All.fig', concentration, FittingTypeS, Mini_Trajlength, R_thresholdS));
