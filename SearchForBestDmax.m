...
input_path=('/Users/rosslab/Documents/Ross Lab/MTT&Spot-On analysis/20200813-0824_viscosity scaling MTT+spot-on analysis/raw/50gly_50ms/');
output_path_Name = sprintf('/Users/rosslab/Documents/Ross Lab/MTT&Spot-On analysis/20200813-0824_viscosity scaling MTT+spot-on analysis/raw/50gly Search for best Dmax/Dmax=[2:2:30]/50gly mat file Dmax=%d Gap=3/', D_in(n));
...
EmissionWavelength = 647; % wavelength in nm; consider emission max and filter cutoff
ExposureTime = 80; % in milliseconds
NumDeflationLoops = 0; % Generaly keep this to 0; if you need deflation loops, you are imaging at too high a density;
MaxExpectedD = D_in(n); % The maximal expected diffusion constant for tracking in units of um^2/s;
NumGapsAllowed = 3; % the number of gaps allowed in trajectories: if=1, no gap [1,2,3,...]; if=2: [1,3,...] meaning 1 gap is allowed; and so on...
%%%%%%%%%%%%%%%%%%%%%%%%%f%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%% DEFINE STRUCTURED ARRAY WITH ALL THE SPECIFIC SETTINGS FOR LOC AND TRACK
% imaging parameters
impars.PixelSize=0.1067; % um per pixel
